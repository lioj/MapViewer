...
Signal<MapViewer::TZ>& MapViewer::time_zone(const Coordinate& pos, bool ajax) {
    ...
    if (js() && ajax) {
        ...
        << "tz_data+=is_st;"
        << "tz_data+='&'+data.countryName+' '+data.timezoneId;}"
...
void MapViewer::tz_data_parser(const std::string& data) {
    int tz;
    bool is_st;
    std::string country_name, time_zone_id;
    if (data != "") {
        std::vector<std::string> v0;
        std::vector<std::string> v;
        std::vector<std::string> v1;
        boost::split(v0, data, boost::is_any_of("&"));
        boost::split(v, v0[0], boost::is_any_of(" "));
        boost::split(v1, v0[1], boost::is_any_of(" "));
    ...
    TZ time_zone = {tz, is_st, country_name, time_zone_id};
...
void MapViewer::tz_data_parser(const boost::system::error_code& e,
    ...
    if (!e) {
        ...
        try {
            ...
            tz = {-13, false, "undefined", "undefined"};
            tz_signal_->emit(tz);
            return;
        }
        try {
            ...
            tz.country_name = obj.get("countryName").toString().orIfNull("undefined");
            tz.tz_id = obj.get("timezoneId").toString().orIfNull("undefined");
        } catch(...){
            tz = {-13, false, "undefined", "undefined"};
            tz_signal_->emit(tz);
            return;
